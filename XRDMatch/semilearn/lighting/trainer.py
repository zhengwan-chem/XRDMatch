...
class Trainer:
    ...
    def fit(self, train_lb_loader, train_ulb_loader, eval_loader, pred_loader):
...
#           self.algorithm.save_model('latest_model.pth', self.save_path)
            ...
            # best
            if result['f1'] > best_eval_acc:
                best_eval_acc = result['f1']
                best_epoch = epoch      
                y_pred, y_logits, y_true = self.predict(pred_loader,return_gt=True)
                pred_f1 = f1_score(y_true, y_pred, average='macro')
                print(1,best_eval_acc,file=file_pre)
                print(2,pred_f1,file=file_pre)          
    ...
    def evaluate(self, data_loader, use_ema_model=False):
        ...
        if cf_mat[0,0] > 0.6 and cf_mat[1,1] > 0.6:
            ...
            print(cf_mat,file=f)
#        f1 = ((cf_mat[0,0]+cf_mat[1,1])/2-abs(cf_mat[0,0]-cf_mat[1,1])/3)*top1
